...
class QuadInputBinaryClassificationLightningModule(pl.LightningModule):
    def __init__(self, config):
        ...
        if str(config['optim']['cw']).lower() == "no":
            ...
            pos_weight = get_pos_weight(config['data']['csv_file'])
...
if __name__ == "__main__":
    ...
    early_stop_callback = EarlyStopping(
        monitor='val_auprc',
        mode='max',
        patience=config['train'].get('early_stopping_patience', 25),
        min_delta=config['train'].get('early_stopping_min_delta', 0.0005),
        verbose=True
    )
