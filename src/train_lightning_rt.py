...
from dataset import get_default_transform_quad, get_validation_transform_quad, quad_image_collate_fn, QuadImageDataset, get_pos_weight
...
class QuadInputBinaryClassificationLightningModule(pl.LightningModule):
    def __init__(self, config):
        ...
        self.classifier = Classifier(d_model=768, num_classes=num_classes)
        self.model = SingleScanModelQuad(self.backbone, self.classifier)


        if str(config['optim']['cw']).lower() == "no":
            self.criterion =  nn.CrossEntropyLoss()
        else:
            print(" Positive class weights - Not Available")
    ...
    def training_step(self, batch, batch_idx):
        ...
        probs = torch.softmax(y_hat_logits, dim=1)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        probs = torch.softmax(y_hat_logits, dim=1)
        output = {'val_loss': loss.detach(), 'y_true': y.detach(), 'y_pred_logits': y_hat_logits.detach(), 'y_pred_probs': probs.detach()}
    ...
    def on_validation_epoch_end(self):
        ...
        all_y_pred_logits = torch.cat([o['y_pred_logits'] for o in self.validation_step_outputs])
        all_y_pred_probs = torch.cat([o['y_pred_probs'] for o in self.validation_step_outputs])


        all_preds = torch.argmax(all_y_pred_logits, dim=1)

        self.val_accuracy.update(all_y_pred_probs, all_y_true)
        self.val_precision.update(all_y_pred_probs, all_y_true)
        self.val_recall.update(all_y_pred_probs, all_y_true)
        self.val_f1.update(all_y_pred_probs, all_y_true)
        self.val_auroc.update(all_y_pred_probs, all_y_true)
